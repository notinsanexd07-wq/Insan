...
export default function App() {
  ...
  const [config, setConfig] = useState<SiteConfig>(() => {
    try {
      ...
      if (saved) {
        ...
        const savedVersion = parsed.settings?.configVersion || '';
        const defaultVersion = DEFAULT_CONFIG.settings.configVersion || '';

        if (savedVersion === defaultVersion && parsed.theme && parsed.hero) {
          parsed.contact = { ...DEFAULT_CONFIG.contact, ...parsed.contact };
          return parsed;
        } else {
          console.log('Saved config version mismatch. Reverting to deployed DEFAULT_CONFIG.');
          localStorage.removeItem('notinsane_portfolio_config');
        }
      }
    } catch (e) {
  ...
  // Auto-sync configuration to server backend (only when modified in this session)
  useEffect(() => {
    if (!isDirty) return;
    ...
    const saveToServer = async () => {
      try {
        ...
        if (!response.ok) {
          ...
          console.log('Successfully synced config to server');
          setIsDirty(false);
        }
      } catch (err) {
    ...
    // Debounce slightly to avoid excessive writes during rapid configuration edits
